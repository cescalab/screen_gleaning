...
public class Main implements TSDRLibrary.FrameReadyCallback, TSDRLibrary.IncomingValueCallback, TSDRSourceParamChangedListener, OnTSDRParamChangedCallback {
	...
	private volatile boolean emage = false;
	...
	private volatile String emageFilename = "";
	private volatile String emageFilenameComplete = "";
	
	private int ready = 1;
	private int takeEmage = 0;
	private int numEmages = 10;
	...
	private void connectToServer() {
		...
		new Thread(new Runnable() {
    			public void run() {
					...
					try {
						WebSocketClient mWs = new WebSocketClient(new URI("ws://127.0.0.1:8081/2")) {
						@Override
						public void onMessage(String message) {
							...
							try {
								...
								emageFilename = jsonArray.get(1).toString();
								numEmages = 1;
								takeEmage = 1;
						...
						// Counter is the total number of emages taken
						while (counter < 400) {
							sleep(3000);
							if (ready == 1) {
								ready = 0;
								...
								if (numEmages >= 0) {
									emageFilenameComplete = emageFilename + Integer.toString(numEmages);
									numEmages--;
									counter++;
									emage = true;
								} else {
	...
	private void initialize() {
		...
		mntmConnect = new JMenuItem("Connect to server");
		mntmConnect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				connectToServer();
			}
		});
		mnTweaks.add(mntmConnect);
	...
	public void onFrameReady(TSDRLibrary lib, BufferedImage frame) {
		if (emage) {
			...
			try {
				...
				final File outputfile = new File("out/" + emageFilenameComplete);
